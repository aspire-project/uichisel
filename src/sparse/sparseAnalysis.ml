...
sig
  ...
  val perform : Spec.t -> Global.t -> Global.t * Table.t * Table.t
	val run : update_mode -> Spec.t -> Table.A.t -> Table.B.t * Global.t -> Table.B.t * Global.t
end
...
(*          ("dugraph-inter", DUGraph.to_json_inter dug access);*)
    ...
    begin
      prerr_memory_usage ();
      my_prerr_endline ("#Nodes in def-use graph : " ^ i2s (DUGraph.nb_node dug));
      my_prerr_endline ("#Locs on def-use graph : " ^ i2s (DUGraph.nb_loc dug));
    end
  ...
  =fun spec global ->
    ...
    let dug = StepManager.stepf false "Def-use graph construction" SsaDug.make (global, access, spec.Spec.locset_fs) in
		(* reset cache info *)
		let _ = SsaDug.reset_cache () in
		let _ = Hashtbl.clear def_locs_cache in  
