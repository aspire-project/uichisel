...
=fun pid icfg ->
  ...
  { icfg with cfgs = BatMap.remove pid icfg.cfgs ;
		globals = List.filter (fun g -> 
			match g with
			| GVarDecl (vi, _) -> not ((String.compare vi.vname pid) = 0)   
			| GFun (fd, _) -> not ((String.compare fd.svar.vname pid) = 0)
			| _ -> true
		) icfg.globals; 
...
let num_of_nonskip_nodes : t -> int 
= fun icfg -> 
	let nodes = nodesof icfg in 
	List.filter (fun node -> 
		let pid = Node.get_pid node in
		if (String.compare pid global_proc) = 0 then false 
		else  
  		let cmd = cmdof icfg node in
  		match cmd with 
