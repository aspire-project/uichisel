...
module Cmd = struct
  ...
  =fun c ->
    ...
    | Cif (e,b1,b2,loc) -> "if"
    | Cassume (e,loc,_) -> "assume(" ^ s_exp e ^ ")"
    | CLoop _ -> "loop"
    | Casm _ -> "asm"
    | Cskip -> "skip"
...
module Path_Checker = Graph.Path.Check(GDom) 
...
type t = {
  ...
  scc_list          : node list list;
	dom_path_checker  : Path_Checker.path_checker option 
}
...
let is_dom : t -> node -> node -> bool 
= fun g n1 n2 ->
	let checker = 
		match g.dom_path_checker with 
		| None -> raise (Failure "IntraCfg.is_dom: dom_tree is not computed")
		| Some checker -> checker 
	in 
	Path_Checker.check_path checker n1 n2  
	
let children_of_dom_tree node g =
  NodeSet.remove node (NodeSet.of_list (try G.succ g.dom_tree node with _ -> []))
...
=fun n g ->
  {g with graph = G.remove_vertex g.graph n ;
          ...
          dom_tree = G.remove_vertex g.dom_tree n } (* TODO: connect preds and succs *)
      ...
      | Cmd.Cif (e,tb,fb,loc) ->
        ...
        let _ = assert (List.length succs = 1 || List.length succs = 2) in
          if List.length succs = 2 then (* normal case *)
            ...
            let tassert = Cmd.Cassume (e,loc,true) in
            let fassert = Cmd.Cassume (UnOp (LNot,e,Cil.typeOf e),loc,false) in
...
let make_init_loop fd lv exp loc entry f g =
  ...
  let assume_cmd = Cmd.Cassume (cond, loc, true) in
  let g = add_cmd assume_node assume_cmd g in
  let g = add_edge skip_node assume_node g in
  let nassume_node = Node.make () in
  let nassume_cmd = Cmd.Cassume (Cil.UnOp (Cil.LNot, cond, Cil.intType), loc, false) in
    ...
    fold_node (fun n g ->
      ...
      | Cmd.Cassume (e, loc, b) ->
          ...
          | (e, l) ->
            ...
            let g = add_cmd empty_node Cmd.Cskip g in
            let (node, g) = generate_sallocs l loc empty_node g in
            let cmd = Cmd.Cassume (e, loc, b) in
            let g = add_cmd last_node cmd g in
            let g = add_edge node last_node g in
              replace_node_graph n empty_node last_node g)
...
=fun g ->
  ...
  let dom_fronts =
    ...
    {g with dom_tree = dom_tree;
            dom_fronts = dom_fronts; 
						dom_path_checker = Some (Path_Checker.create dom_tree)}
